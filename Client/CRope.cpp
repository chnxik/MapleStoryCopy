...
void CRope::BeginOverlap(CCollider* _pOther)
{
}

void CRope::OnOverlap(CCollider* _pOther)
{
	...
	if (pPlayer)
	{
		if (pPlayer->GetLiftObj() == nullptr)
		{
			if (ISHOLD(KEY::UP) && (_pOther->GetGrndPt().y >= GetLdot().y + 20.f))
			{
				...
				pPlayer->ChangeState(CHANGESTATE(CPlayerState)::rope);
			...
			if (ISHOLD(KEY::DOWN) && (_pOther->GetGrndPt().y < GetLdot().y + 20.f))
			{
				pPlayer->SetLiftObj(this);
				pPlayer->ChangeState(CHANGESTATE(CPlayerState)::rope);
				pPlayer->SetPos(Vec2(GetCenterx(), pPlayer->GetPos().y + 15.f));
			}
		}
	}
}

void CRope::EndOverlap(CCollider* _pOther)
{
	CPlayer* pPlayer = dynamic_cast<CPlayer*>(_pOther->GetOwner());
